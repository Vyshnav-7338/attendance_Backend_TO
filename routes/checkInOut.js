...
router.put(
  "/api/checkout",
  upload.single("checkOutPhoto"),
  authenticate,
  async (req, res) => {
    try {
      const userId = req.userId;

      const latestCheckIn = await CheckInOut.findOne({
        userId,
        isCheckedIn: true,
      }).sort({ createdAt: -1 });
...
router.get("/api/checkInOut/status", authenticate, async (req, res) => {
  try {
    ...
    const latestStatus = await CheckInOut.findOne({ userId }).sort({
      createdAt: -1,
    });
    ...
    if (!latestStatus) {
      return res
        .status(404)
        .json({ error: "No check-in/check-out record found" });
    }
