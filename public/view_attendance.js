document
  .getElementById("fetchAttendanceBtn")
  .addEventListener("click", async function () {
    ...
    try {
      const response = await fetch(
        `http://68.178.163.246:3004/api/attendance?date=${date}`,
        {
          headers: {
...
function populateAttendanceTable(data) {
  ...
  data.forEach((entry) => {
    ...
    const checkInTime = entry.checkInTime
      ? new Date(entry.checkInTime).toLocaleTimeString("en-IN", indianOptions)
      : "Nill";
    ...
    const checkOutTime = entry.checkOutTime
      ? new Date(entry.checkOutTime).toLocaleTimeString("en-IN", indianOptions)
      : "Nill";
